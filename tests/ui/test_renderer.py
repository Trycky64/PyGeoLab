...
def test_renderer_draws_supported_document_geometry_without_mutation(
    qapp: QApplication,
) -> None:
    del qapp
    document = Document()
...
def test_renderer_draws_visible_snap_indicator(qapp: QApplication) -> None:
    del qapp
    ...
    Renderer.render_snap_indicator(
        painter,
        Viewport(scale=40, width=120, height=100),
        palette,
        Point2D(0, 0),
    )
    painter.end()

    assert image.pixelColor(QPoint(60, 43)).alpha() > 0
