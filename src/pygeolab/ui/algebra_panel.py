...
from PySide6.QtWidgets import QMenu, QTreeWidget, QTreeWidgetItem, QWidget, QVBoxLayout
...
class AlgebraPanel(QWidget):
    ...
    def _show_context_menu(self, position: object) -> None:
        ...
        if chosen is visibility:
            self._execute_command(ChangeVisibilityCommand(self._document, object_id, not obj.visible))
        elif chosen is delete:
            self._execute_command(DeleteObjectCommand(self._document, object_id))

    def _iter_object_items(self):
    ...
    def _category(geometry: object) -> str:
        if isinstance(geometry, Point2D): return "Points"
        if isinstance(geometry, (Line2D, Segment2D, Ray2D, Vector2D)): return "Lignes"
        if isinstance(geometry, Circle2D): return "Cercles"
        if isinstance(geometry, Polygon2D): return "Polygones"
        if isinstance(geometry, float): return "Nombres"
        return "Objets"
    ...
    def _format_value(geometry: object, valid: bool) -> str:
        if not valid or geometry is None:
            return "indéfini"
        if isinstance(geometry, Point2D): return f"({geometry.x:.3g}, {geometry.y:.3g})"
        if isinstance(geometry, Segment2D): return f"longueur {geometry.length:.3g}"
        if isinstance(geometry, Circle2D): return f"r = {geometry.radius:.3g}"
        if isinstance(geometry, Polygon2D): return f"aire = {geometry.area:.3g}"
        if isinstance(geometry, float): return f"{geometry:.6g}"
