...
from pygeolab.commands import ChangeStyleCommand, ChangeVisibilityCommand, Command, RenameObjectCommand
...
class PropertiesPanel(QWidget):
    ...
    def __init__(self, document: Document, execute_command: Callable[[Command], None], parent: QWidget | None = None) -> None:
        ...
        self._width = QDoubleSpinBox(self); self._width.setRange(0.5, 20); self._width.setSingleStep(0.5)
        self._point_size = QDoubleSpinBox(self); self._point_size.setRange(1, 30)
        self._line_style = QComboBox(self); self._line_style.addItems(["solid", "dash", "dot"])
        self._label = QCheckBox(self)
        self._opacity = QDoubleSpinBox(self); self._opacity.setRange(0, 1); self._opacity.setSingleStep(0.05)
    ...
    def set_document(self, document: Document) -> None:
        ...
        self._unsubscribe(); self._document = document; self._object_id = None
        self._unsubscribe = document.subscribe(self.refresh); self.refresh()
    ...
    def refresh(self) -> None:
        ...
        for widget in (self._name, self._visible, self._color, self._width, self._point_size, self._line_style, self._label, self._opacity):
            widget.setEnabled(enabled)
        if obj is None:
            return
        self._updating = True
        try:
            self._name.setText(obj.name); self._visible.setChecked(obj.visible)
            self._width.setValue(obj.style.width); self._point_size.setValue(obj.style.point_size)
            self._line_style.setCurrentText(obj.style.line_style); self._label.setChecked(obj.style.show_label)
    ...
    def _rename(self) -> None:
        if self._updating or self._object_id is None: return
        obj = self._document.get(self._object_id); name = self._name.text().strip()
    ...
    def _change_visibility(self, visible: bool) -> None:
        ...
        if visible != obj.visible:
            self._execute_command(ChangeVisibilityCommand(self._document, obj.id, visible))

    def _choose_color(self) -> None:
        if self._object_id is None: return
        obj = self._document.get(self._object_id)
        color = QColorDialog.getColor(parent=self)
        if color.isValid():
            self._execute_command(ChangeStyleCommand(self._document, obj.id, self._style(color=color.name())))

    def _change_style(self, *args: object) -> None:
        del args
        if self._updating or self._object_id is None: return
        obj = self._document.get(self._object_id); style = self._style()
        if style != obj.style:
    ...
    def _style(self, color: str | None = None) -> Style:
        ...
        return Style(color=color or obj.style.color, width=self._width.value(), point_size=self._point_size.value(), line_style=self._line_style.currentText(), show_label=self._label.isChecked(), fill_opacity=self._opacity.value())
